...
echo 'starting as exec "$SCRIPTPATH"/bin.js start -- --friends.hops "$HOPS" --caps.shs "$CAPS" --path "$DIR" --port "$PORT" --ws.port "$WS_PORT"'
...
DEBUG='ssb:replication-scheduler,ssb:db2,ssb:conn*' exec "$SCRIPTPATH"/bin.js start -- --no-conn.autostart --friends.hops "$HOPS" --caps.shs "$CAPS" --path "$DIR" --port "$PORT" --ws.port "$WS_PORT"
